...
class MPickPlaceController:
    ...
    def forward(
        ...
        if self._event == 0:  # to init height
            self.object_init_pose = picking_position
            position_target = np.array(
                [
                    picking_position[0] + end_effector_offset[0],
                    picking_position[1] + end_effector_offset[1],
                    picking_position[2] + end_effector_offset[2] + 0.1,
                ]
            )
        ...
        elif self._event == 1: # lower ee down to object

            a = self._mix_sin(max(0, self._t))
            h = self._combine_convex(self.object_init_pose[2] + 0.1, self.object_init_pose[2], a)
        ...
        elif self._event == 2: # close gripper
            target_joint_positions = self._gripper.forward(action="close")
            # [None, None, None, None, None, None, 1.2, None, None, 1.2, None, None]

        elif self._event == 3:
            a = self._mix_sin(max(0, self._t))
            h = self._combine_convex(self.object_init_pose[2], self.object_init_pose[2]+0.1, a)
            # print(f"a: {a}")
            # print(f"h: {h}")
            # print(f"picking_position[2] + end_effector_offset[2]: {picking_position[2] + end_effector_offset[2]}")
            position_target = np.array(
                [
                    picking_position[0] + end_effector_offset[0],
                    picking_position[1] + end_effector_offset[1],
        ...
        elif self._event == 4: # to init pose
            init_pose= np.array([3.1415927, -2.871760, 2.799204, -3.072348, -1.581982, -0.000120])
            interpolated_positions = self.interpolate_positions(current_joint_positions[0:6], init_pose, self._t)
...
#################################  PLACE  ##################################33
        elif self._event == 5: # to place position (with z +0.01)
            position_target = np.array(
                [
                    placing_position[0] + end_effector_offset[0],
                    placing_position[1] + end_effector_offset[1],
                    placing_position[2] + end_effector_offset[2] + 0.01,
                ]
            )
            if end_effector_orientation is None:
                end_effector_orientation = euler_angles_to_quat(np.array([0, np.pi, 0]))
        ...
        elif self._event == 6:    # open gripper
            target_joint_positions = self._gripper.forward(action="open")

        elif self._event == 7:    # to init pose
            init_pose= np.array([3.1415927, -2.871760, 2.799204, -3.072348, -1.581982, -0.000120])
            interpolated_positions = self.interpolate_positions(current_joint_positions[0:6], init_pose, self._t)
            interpolated_positions = np.append(interpolated_positions, [None, None, None, None, None, None])
            target_joint_positions = ArticulationAction(joint_positions= interpolated_positions,)
    ...
    def interpolate_positions(self,current_joint_positions, joints_default_positions, t):
        ...
        interpolated_positions = (1 - alpha) * current_joint_positions + alpha * joints_default_positions
